...
def process_batch(batch_dir, output_dir, top_features, window_size=1200, step_size=300):
    ...
    try:
        ...
        output_file = os.path.join(output_dir, f"{batch_name}.parquet")
...
def main():
    ...
    for data_type in data_types:
        for component in components:
            ...
            output_dir = f"Data/processed/lsmt_statisticalFeatures/statistic_features_filtered/{data_type}/{component}"
            os.makedirs(output_dir, exist_ok=True)
            
            # Process each batch
            batch_windows = 0
            for batch_idx, batch_dir in enumerate(batch_dirs):
                logger.info(f"Processing batch {batch_idx+1}/{len(batch_dirs)} for {batch_dirs}")
