...
def load_config() -> dict:
    ...
    config_path = Path("configs/soft_label_slidingWindow.yaml")
...
def process_segment(
    ...
    Returns:
        Tuple of (windows, soft_labels, segment_ids, timestamps, stats)
        where soft_labels are continuous values between 0 and 1 representing
        the proportion of the window that overlaps with anomaly periods
    """
    ...
    while start_idx + window_size <= len(segment_df):
        ...
        if len(window_data) == window_size:
            ...
            # Use the overlap ratio directly as the soft label
            windows.append(window_data)
            labels.append(overlap_ratio)
...
def create_sliding_windows(
    ...
    """
    Create sliding windows from segment data and label them based on anomaly overlap ratio.
    ...
    Returns:
        ...
        - soft_labels: Array of continuous labels between 0 and 1 representing the proportion
          of each window that overlaps with anomaly periods
    ...
    # Log statistics about the soft labels
    label_stats = {
    ...
    logger.info(f"Soft label statistics: {label_stats}")
...
def process_component_data(
    ...
    logger.info(f"Processing {component} {data_type} data with soft labels")
    ...
    for batch_idx, batch_dir in enumerate(batch_dirs):
        ...
        # Process this batch
        logger.info(f"Creating sliding windows with soft labels for {batch_name} {component} {data_type}")
        windows, soft_labels = create_sliding_windows(
            df,
            component,
        ...
        if len(windows) > 0:
            ...
            logger.info(f"Saved {len(windows)} windows with soft labels to temporary file {batch_file}")
        
        # Free memory
        del df, windows, soft_labels
        gc.collect()
