...
from src.pca.dataLoader import get_pc_dataloaders
...
def load_config(config_path=None):
    ...
    if config_path is None:
        config_path = Path("configs/train_transform_pc.yaml")
...
def train_epoch(model, data_loader, optimizer, criterion, device, scheduler=None):
    ...
    for batch_idx, batch_data in enumerate(progress_bar):
        ...
        # 新代码:
        inputs = batch_data['features'].to(device)  # [batch_size, seq_len, feature_dim]
        labels = batch_data['labels'].to(device)    # [batch_size, seq_len]
        ...
        # Get predictions
        _, preds = torch.max(outputs, 2)  # [batch_size, seq_len]
...
def evaluate(model, data_loader, criterion, device, config, print_samples=True):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            probs = torch.softmax(outputs, dim=2)  # [batch_size, seq_len, num_classes]
            ...
            all_preds.append(preds.cpu().numpy())
            all_labels.append(labels.cpu().numpy())
            all_probs.append(anomaly_probs.cpu().numpy())
            
            # Print sample predictions (only once)
            if print_samples and batch_idx == 0:
                sample_idx = 0
                logger.info(f"Sample predictions for first sequence in batch:")
                sample_preds = preds[sample_idx].cpu().numpy()
                sample_labels = labels[sample_idx].cpu().numpy()
                sample_probs = anomaly_probs[sample_idx].cpu().numpy()
                
                for t in range(min(seq_len, 20)):  # Print first 20 timesteps
                    logger.info(f"t={t}: True={sample_labels[t]}, Pred={sample_preds[t]}, Prob={sample_probs[t]:.4f}")
    ...
    # 打印形状信息以进行调试
    logger.info(f"Shape before flatten - all_preds: {all_preds.shape}, all_labels: {all_labels.shape}, all_probs: {all_probs.shape}")
    ...
    # 确保维度一致
    logger.info(f"Shape after flatten - flat_preds: {flat_preds.shape}, flat_labels: {flat_labels.shape}, flat_probs: {flat_probs.shape}")
    
    # 验证维度一致性
    assert flat_labels.shape == flat_probs.shape, f"Shape mismatch: labels {flat_labels.shape} vs probs {flat_probs.shape}"
...
def main(args):
    ...
    # 新代码:
    data_loaders = get_pc_dataloaders(
        data_path=config['paths'].get('pc_data_path', "Data/pc/pc_features_train.parquet"),
        batch_size=config['training']['batch_size'],
        seq_length=config['data'].get('seq_length', 60),
        stride=config['data'].get('stride', 1),
        val_ratio=config['data'].get('val_ratio', 0.15),
        num_workers=config['training']['num_workers'],
        standardize=config['data'].get('standardize', False)
    )
    
    # 修改这一部分: 适应新的数据加载器返回格式
    # 获取样本批次以确定输入维度
    sample_batch = next(iter(data_loaders['train']))
    input_features = sample_batch['features']  # [batch_size, seq_len, feature_dim]
    input_dim = input_features.shape[2]  # 特征维度
    seq_len = input_features.shape[1]  # 序列长度
