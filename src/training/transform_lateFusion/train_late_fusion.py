"""
Training script for Transformer Late Fusion model for energy anomaly detection.

This script trains a Transformer model with late fusion of statistical features
using PyTorch's nn.TransformerEncoder for energy anomaly detection.
"""
...
def load_config(config_path=None):
    ...
    if config_path is None:
        config_path = Path("configs/transformer_lateFusion.yaml")
...
def train_epoch(model, data_loader, optimizer, criterion, device, scheduler=None):
    ...
    Args:
        model: Transformer late fusion model
    ...
    for batch_idx, (data, stat_features, labels) in enumerate(pbar):
        data, stat_features, labels = data.to(device), stat_features.to(device), labels.to(device)
        data = data.float()
        stat_features = stat_features.float()

        optimizer.zero_grad()
        outputs, _ = model(data, stat_features)
...
def evaluate(model, data_loader, criterion, device, config, print_samples=True, find_optimal_threshold=True):
    ...
    with torch.no_grad():
        for batch_idx, (data, stat_features, labels) in enumerate(tqdm(data_loader, desc="Evaluating")):
            data, stat_features, labels = data.to(device), stat_features.to(device), labels.to(device)
            data = data.float()
            stat_features = stat_features.float()
    ...
    print("Max anomaly prob:", all_scores.max().item())
    print("Mean anomaly prob:", all_scores.mean().item())
    print("Mean anomaly prob:", np.mean(all_labels))
...
def main(args):
    """
    Main function to train the Transformer Late Fusion model.
    ...
    experiment_name = f"transformer_late_fusion_{timestamp}"
    ...
    sample_batch, sample_stat_features, _ = next(iter(data_loaders['train']))
    input_dim = sample_batch.shape[2]  # [batch_size, seq_len, input_dim]
    seq_len = sample_batch.shape[1]
    stat_features_size = sample_stat_features.shape[1]  # [batch_size, stat_features_size]
    ...
    model = TransformerLateFusionModel(
        ...
        stat_features_size=stat_features_size
    )
    ...
    if config['training']['use_focal_loss']:
        ...
        if config['training']['use_class_weights']:
            ...
            all_labels = []
            for _, _, labels in data_loaders['train'].dataset:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train Transformer Late Fusion model for energy anomaly detection")
