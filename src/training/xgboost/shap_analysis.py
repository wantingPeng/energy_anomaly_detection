"""
SHAP Analysis for XGBoost Anomaly Detection.

This script performs SHAP (SHapley Additive exPlanations) analysis on a trained
XGBoost model to explain model predictions and feature importance.
...
class XGBoostSHAPAnalyzer:
    """
    SHAP analyzer for XGBoost anomaly detection.
    ...
    def __init__(self, model, X, feature_names, output_dir, y=None, max_display=20, random_state=42):
        ...
        logger.info("Initializing SHAP TreeExplainer for XGBoost...")
    ...
    def plot_waterfall_examples(self, n_examples=5, example_type='auto'):
        """
    ...
    def generate_full_analysis(self, sample_size=None, sampling_strategy='stratified', 
                             top_n_dependence=5, n_waterfall_examples=5):
        """
...
def main(args):
    ...
    """
    logger.info("\n" + "=" * 60)
    logger.info("XGBOOST SHAP ANALYSIS")
    logger.info("=" * 60)
    ...
    # Load model
    logger.info("\n--- Loading Model ---")
    model_wrapper = XGBoostAnomalyDetector(config)
    model_wrapper.load_model(args.model_path)
    logger.info(f"Model loaded successfully")
    logger.info(f"  Optimal threshold: {model_wrapper.optimal_threshold}")
    ...
    logger.info(f"\n--- Using {dataset_name.upper()} set for SHAP analysis ---")
    logger.info(f"  Samples: {len(X_analysis)}")
    logger.info(f"  Anomalies: {y_analysis.sum()} ({y_analysis.mean():.4f})")
    ...
    analyzer = XGBoostSHAPAnalyzer(
        model=model_wrapper.model,  # Get the xgboost.Booster model
        X=X_analysis,
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("XGBOOST SHAP ANALYSIS SUMMARY\n")
        f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SHAP analysis for XGBoost anomaly detection"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/xgboost/shap_analysis",
